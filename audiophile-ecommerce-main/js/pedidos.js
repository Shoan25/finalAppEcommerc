...
var intc=0;
for (var [key, value] of ormap) {
    
if(value.grandTotal>0){
 contenido.innerHTML+=`<div class="modal__content">
                       
 <div class="modal__purchased-products purchased-products">
   <div class="purchased-products__details">
     
 <p class="modal__text">Order ID #${value.id.replace("order",' ')}</p>
     <div class="products-list products-list--collapsed">
       <ul class="products-list__list" id="prodListFinal${intc}">
         
       </ul>

       <div class="products-list__toggle-wrapper">
         <button class="products-list__toggler" type="button" id="buttonShowMoreFinal${intc}" onClick="">and 2 other item(s)</button>
       </div>
     </div>
   </div>

   <div class="purchased-products__grand-total">
     <p class="purchased-products__grand-total-text">Grand Total</p>
     <p class="purchased-products__grand-total-value" id="grandTotalFinal">$ ${formatoMoneda(value.grandTotal)}</p>
     </br>
     <p class="purchased-products__grand-total-text">Adress</p>
     <p class="purchased-products__grand-total-value" id="grandTotalFinal">${value.adress}</p>
   </div>
 </div>`
 ...
 let buttonShowMoreFinal=document.getElementById('buttonShowMoreFinal'+intc)
 const prodListFinal = document.getElementById('prodListFinal'+intc);
 prodListFinal.innerHTML=``



 var prodsFinal1= new Map(JSON.parse(value.productsMap));
 var totalSummary=0;
 var qp=0;
    for (var [pathProd, quantity] of prodsFinal1) {
      ...
      <div class="product-cart-card__info">
        ...
        <button id="2" class="elemento" onclick="devolverProducto('${pathProd}','${quantity}')">Devolver</button>
      </div>
    </li>`
  ...
  var prodsFinal = prodsFinal1.size;
  if(prodsFinal<2)
{
...
intc++
 ...
 function devolverProducto(pathProd, quantity){
  let path = pathProd.split(',');
  //path[0] is category
  //path[1] is product.id
  let mapWhereProducIs = categoriesMap.get(path[0]);
  ...
  document.getElementById('contenidoGeneral').innerHTML=`<div class="product-cart-card__img-wrapper">
  <img class="product-cart-card__img" src="${productToAdd.srcImgSmall}" alt="" width="48" height="48" srcset="${productToAdd.srcImgSmall} 1x, ${productToAdd.srcImgSmallx2} 2x">
</div>
...
<div class="product-cart-card__info">
  ...
  <div class="number-controls" data-item-count="0">
                ...
                <button class="number-controls__button number-controls__button--increment" id="incrementButton" onclick="increment(${quantity})" type="button" aria-label="Add one">+</button>
   </div>
   
</div>

<div><button id="1" class="elemento" onclick="devolverProductoDefinitivo('${productToAdd.title}','${quantity}')">Devolver</button>
<button id="2" class="elemento" onclick="reemplazarProducto('${productToAdd.title}','${quantity}')">Reemplazar</button></div>`

 }

 let mapaDevs = new Map();
 
 function devolverProductoDefinitivo(nombre, cantidad){
  if(parseInt(document.getElementById('currentItemCount').innerText)<=parseInt(cantidad)&&parseInt(document.getElementById('currentItemCount').innerText)>0){
  if(mapaDevs.get(nombre)){
   /* */
  
    if(mapaDevs.get(nombre)<cantidad){
      mapaDevs.set(nombre,mapaDevs.get(nombre)+1)
      enviarCorreo("Tal y como lo solicito se hara el reintegro de su dinero al metodo usado, por favor devuelva: \n"+nombre+" x"+parseInt(document.getElementById('currentItemCount').innerText)+"\n\n Tan pronto recibamos los productos vera reflejado el reintegro en 5 dias habiles.\n\nDirijase a un DHL con la siguiente guia prepagada: CO234823.PDF")
      if(mapaDevs.get(nombre)<cantidad) alert("Aun tienes mas unidades que devolver en caso de que necesites");
    }else alert("No tienes mas productos que devolver, ya tramitaste todos");
  }
    else {
      if(parseInt(document.getElementById('currentItemCount').innerText)<=cantidad)mapaDevs.set(nombre,parseInt(document.getElementById('currentItemCount').innerText))
      enviarCorreo("Tal y como lo solicito se hara el reintegro de su dinero al metodo usado, por favor devuelva: \n"+nombre+" x"+parseInt(document.getElementById('currentItemCount').innerText)+"\n\n Tan pronto recibamos los productos vera reflejado el reintegro en 5 dias habiles.\n\nDirijase a un DHL con la siguiente guia prepagada: CO234823.PDF")
      if(mapaDevs.get(nombre)<cantidad) alert("Aun tienes mas unidades que devolver en caso de que necesites");
    }
 ...
 function reemplazarProducto(nombre, cantidad){
  if(parseInt(document.getElementById('currentItemCount').innerText)<=parseInt(cantidad)&&parseInt(document.getElementById('currentItemCount').innerText)>0){
    if(mapaDevs.get(nombre)){
      if(mapaDevs.get(nombre)<cantidad){
        mapaDevs.set(nombre,mapaDevs.get(nombre)+1)
        enviarCorreo("Tal y como lo solicito se hara el reemplazo de su/s producto/s, por favor devuelva: \n"+nombre+" x"+cantidad+"\n\n Tan pronto recibamos los productos se le enviara las unidades nuevas.\n\nDirijase a un DHL con la siguiente guia prepagada: CO90347804.PDF")
        if(mapaDevs.get(nombre)<cantidad) alert("Aun tienes mas unidades que devolver en caso de que necesites");
      }else alert("No tienes mas productos que devolver, ya tramitaste todos");
      ...
      if(parseInt(document.getElementById('currentItemCount').innerText)<=cantidad)mapaDevs.set(nombre,parseInt(document.getElementById('currentItemCount').innerText))
      enviarCorreo("Tal y como lo solicito se hara el reemplazo de su/s producto/s, por favor devuelva: \n"+nombre+" x"+cantidad+"\n\n Tan pronto recibamos los productos se le enviara las unidades nuevas.\n\nDirijase a un DHL con la siguiente guia prepagada: CO90347804PDF")
      if(mapaDevs.get(nombre)<cantidad) alert("Aun tienes mas unidades que devolver en caso de que necesites");
    } 
...
function enviarCorreo(mensaje){
 ...
 if(parseInt(document.getElementById('currentItemCount').innerText)<parseInt(quantity)){
  document.getElementById('currentItemCount').innerText=parseInt(document.getElementById('currentItemCount').innerText)+1
 }else alert("Solo adquiriste "+quantity+" unidad/es y eso es lo que puedes tramitar")


 }

 function decrement(){
  if(parseInt(document.getElementById('currentItemCount').innerText)>0){
    document.getElementById('currentItemCount').innerText=parseInt(document.getElementById('currentItemCount').innerText)-1
   }
