    ...
    const MINIMUM_ITEM_COUNT = 1;
      ...
      if (elCheckoutForm) {
        ...
        var templateParams = {
          ...
          message: "El codigo para continuar su compra es: "+pinCorrecto,
        };
...
let correoContenido = "\nProducto(s) adquirido(s)"
  ...
  qp+=quantity;
  correoContenido+="\n"+productToAdd.title+"   x"+ quantity+ "  unidad(es)"
  }
...
correoContenido+="\nTotal: $"+grandtotal;
  ...
  class Order {
    constructor(id, total, shipping, iva, grandTotal, productsMap, adress) {
      this.id=id;
      this.total = total;
  ...
  adress = document.getElementById('address-input').value.toString()
  let order = new Order("order"+orderCount,totalF, shippingF, ivaF, granTF, JSON.stringify(Array.from(contentCartMap.entries())), adress);
//
...
console.log(ormap)
 ...
 const asunto = "";
 let contenido = "Tu orden con ID: "+orderCount+ ", ha sido creada con exito\n"+"\n";
